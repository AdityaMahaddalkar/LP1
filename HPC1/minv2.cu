...
void compute_min_cpu(float *cpu_input, float *cpu_output, unsigned int n){
    cpu_output[0] = FLT_MAX;
    
    auto start = high_resolution_clock::now();
    for(int i = 0;i < n;i ++){
        cpu_output[0] = min(cpu_output[0], cpu_input[i]);
    }
    auto stop = high_resolution_clock::now();
    auto time_req = duration_cast<microseconds>(stop - start).count();
    cout << endl << " minimum from CPU is : " << cpu_output[0] << endl;
...
int main(){
    ...
    for(unsigned int i = 0;i < n;i ++){
        ...
        if(i == 23842){
            cpu_input[i] = 101.0; // minimum for testing
        }
        else{
            cpu_input[i] = 99.0;
        }
    ...
    //Execute CPU code for minimum
    compute_min_cpu(cpu_input, cpu_output, n);

    //Execute GPU code for minimum
    dim3 grid_size(blocksPerGrid, 1, 1);
    dim3 block_size(numThreadsPerBlock, 1, 1);

    auto start = high_resolution_clock::now();
    compute_min_gpu<<<grid_size, block_size, numThreadsPerBlock * sizeof(float)>>>(device_input, device_output);
    cudaMemcpy(transfer_output, device_output, blocksPerGrid * sizeof(float), cudaMemcpyDeviceToHost);
    // Compute the minimum from the final array
    float minimum = FLT_MAX;
    for(int i = 0;i < blocksPerGrid;i ++){
        minimum = min(minimum, transfer_output[i]); 
    }
    auto stop = high_resolution_clock::now();
    auto time_req = duration_cast<microseconds>(stop - start).count();
    cout << endl << " minimum from GPU is : " << minimum << endl;
